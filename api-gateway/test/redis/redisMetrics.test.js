 ...
 * dans `rateLimitStore.test.js` : c'est ce qui garde la suite à quelques
 * secondes et sans dépendance d'environnement.
 *
 * Portage du fichier homologue de `api-paynoval` (`test/redisMetrics.test.js`),
 * assertions inchangées : c'est le même module qu'on protège. Les deux tests de
 * la fin sont propres à la passerelle — ce sont eux qui verrouillent le câblage
 * local (client partagé, cache applicatif).
 */
...
/* ─────────────── Câblage propre à la passerelle ─────────────── */

/**
 * INVARIANT 8, APPLIQUÉ ICI.
 *
 * La passerelle n'a qu'UNE connexion Redis durable : celle de
 * `services/rateLimitStore.js`, déjà partagée avec le cache de référentiel de
 * change (`fxRulesService`). Les métriques doivent la réutiliser, pas en ouvrir
 * une troisième.
 *
 * Le test lit la source d'`app.js` : il tombe si quelqu'un branche les jauges
 * sur un `new Redis(...)` ou sur un client construit sur place.
 */
test("app.js branche les jauges sur le client de rateLimitStore, pas sur un nouveau", () => {
  const source = fs.readFileSync(path.join(__dirname, "../../src/app.js"), "utf8");

  const bloc = source.slice(
    source.indexOf("registerRedisMetrics(metrics"),
    source.indexOf("registerMongoPoolMetrics(metrics")
  );

  assert.ok(bloc.length > 0, "le câblage doit exister dans app.js");
  assert.match(bloc, /rateLimitStore/);
  assert.match(bloc, /getClient\(\)/);
  assert.ok(!/new\s+Redis\s*\(/.test(bloc));
});

/**
 * SPÉCIFICITÉ PASSERELLE : hit-rate APPLICATIF ≠ hit-rate SERVEUR.
 *
 * `keyspace_hits` compte les lectures vues par le serveur Redis, tous usages
 * confondus (limitation de débit incluse). Les compteurs de `cacheService`
 * comptent ce que le cache de référentiel de change a servi sans relire Mongo.
 * Les deux peuvent diverger complètement : publier l'un pour l'autre ferait
 ...
 * Redis configuré, elle rend `null` — et surtout ne lève pas, sinon `app.js`
 * renoncerait aux jauges alors que Redis va très bien.
 */
test("rateLimitStore.getClient() rend null sans Redis configuré, sans lever", () => {
  const store = require("../../src/services/rateLimitStore");

  const saved = { url: process.env.REDIS_URL, host: process.env.REDIS_HOST };
  ...
  try {
    store.__reset();
    assert.equal(store.getClient(), null);
  } finally {
    store.__reset();
