...
router.all("/pricing/lock", (_req, res) =>
  res.status(403).json({
    success: false,
    message: "Forbidden on public endpoint",
  })
);

// -----------------------------------------------------------------------------
// 4) FX latest/history (READ-ONLY)
// GET /api/v1/public/fx/latest?base&quote (ou from/to)
// GET /api/v1/public/fx/history?base&quote&days
//
// ✅ IMPORTANT:
// Ces endpoints doivent appeler le backend principal directement,
// pas le gateway lui-même.
// -----------------------------------------------------------------------------
...
router.get("/fx/latest", async (req, res) => {
  const base = String(req.query.base || req.query.from || "")
    .toUpperCase()
    .trim();
  const quote = String(req.query.quote || req.query.to || "")
    .toUpperCase()
    .trim();
  ...
  if (!base || !quote) {
    return res.status(400).json({
      success: false,
      message: "base & quote requis (ou from/to)",
    });
  }
  ...
  if (!FX_SERVICE_URL) {
    return res.status(503).json({
      success: false,
      message:
        "FX service unavailable: missing FX_SERVICE_URL or PRINCIPAL_API_BASE_URL",
    });
  }
