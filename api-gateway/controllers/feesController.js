...
// Optionnel : si tu as un logger central
let logger = null;
try {
...
// Helper pour lire la config FX depuis l'env (au lieu de hardcoder la clé)
const FX_API_BASE_URL =
  process.env.FX_API_BASE_URL || 'https://v6.exchangerate-api.com/v6';
const FX_API_KEY = process.env.FX_API_KEY || 'REPLACE_ME'; // mets ta clé dans .env
...
exports.simulateFee = async (req, res) => {
  try {
    ...
    // Par défaut 1% – tu pourras ensuite plugger tes barèmes Fee pour aller plus loin
    let pct = 0.01;
    const providerKey = (provider || '').toLowerCase();
    ...
    // 3.1 – Taux custom admin prioritaire
    ...
    if (customRate) {
      ...
      // 3.2 – Fallback sur l’API du marché
