...
//     'paynoval', 'stripe', 'bank', 'mobilemoney', 'visa_direct', 'stripe2momo', 'flutterwave'
//   ).required(),
...
//     // Ajoute d'autres champs requis ici
//   }),
// };
...
//     if (error) {
//       // Ajoute la devise si possible dans le premier détail plafond
//       let details = error.details.map(d => d.message);
//       // Ajout devise F CFA si c'est un plafond
//       details = details.map(msg => {
//         if (/less than or equal to (\d+)/i.test(msg)) {
//           return msg.replace(
//             /less than or equal to (\d+)/i,
//             (m, p1) => `less than or equal to ${p1} ${currency}`
//           );
//         }
//         return msg;
//       });
...
//                 details: [
//                   `Le plafond journalier autorisé est ${dailyLimit.toLocaleString('fr-FR')} ${currency}.`
//                 ]
//               });
//             }
//           }
...
const baseSchema = {
  funds: Joi.string()
    .valid("paynoval", "stripe", "bank", "mobilemoney", "visa_direct", "stripe2momo", "flutterwave")
    .required(),
...
function validateTransaction(action) {
  return function (req, res, next) {
    // Pour initiate: on garde funds/destination pour allowedFlows,
    // mais le schema et le routing se feront via providerSelected.
    const body = req.body || {};
    ...
    // ✅ on harmonise provider pour les logs et pour les controllers (compat)
    // (mais on garde destination/funds intacts)
