...
function isKybVerified(user = {}) {
  const businessLevel = Number(
    user.businessKYBLevel || user.business?.businessKYBLevel || 0
  );
...
module.exports = async function requireTransactionEligibility(req, res, next) {
  try {
    ...
    const failures = buildEligibilityFailure(mergedUser);
    ...
    req.user = mergedUser;
    req.verifiedUserProfile = mergedUser;
    req.transactionEligibility = {
      ok: true,
      checkedAt: new Date().toISOString(),
      emailVerified: true,
      phoneVerified: true,
      kycVerified: !isBusinessUser(mergedUser),
      kybVerified: isBusinessUser(mergedUser),
      userType: mergedUser.userType || null,
      accountStatus: mergedUser.accountStatus || null,
    };
