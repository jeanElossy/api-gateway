...
//   if (isSocketIoRequest(req)) return next(); // ✅ ne pas casser la socket
...
// // ✅ Proxy socket AVANT auth globale
...
// // ➜ on évite de bloquer admin/support/superadmin
 ...
 * JSON parser global.
 * NOTE:
 * Si un provider webhook futur exige raw body pour signature,
 * il faudra ajouter un express.raw() ciblé AVANT cette ligne
 * pour la route concernée.
 */
app.use(express.json({ limit: "2mb" }));
app.use(express.urlencoded({ extended: true, limit: "2mb" }));
app.use(loggerMiddleware);
