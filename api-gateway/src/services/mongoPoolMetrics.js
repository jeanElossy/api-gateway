 ...
 * ============================================================================
 *
 * ⚠️ FICHIER RÉPLIQUÉ DANS LES TROIS SERVICES
 * (`api-paynoval`, `paynoval-backend`, `api-gateway`). Dépôts séparés, pas de
 * paquet commun — même convention que `metrics.js` et `redisStoreSafety.js`.
 * **Le code est identique au caractère près** ; seuls deux paragraphes de cet
 * en-tête diffèrent, parce qu'ils décrivent la topologie de pool PROPRE à ce
 * dépôt (variable de configuration, nombre de clients). Toute correction du
 * code doit être reportée telle quelle dans les deux autres.
 *
 * ⚠️ L'en-tête d'`api-paynoval` annonce « `api-gateway` s'il ouvre un jour une
 * connexion ». C'était faux au moment où il a été écrit : la passerelle ouvre
 * DEUX connexions Mongo depuis `src/db.js` (`gateway` et `users`).
 *
 * ═══ LA QUESTION À LAQUELLE PERSONNE NE POUVAIT RÉPONDRE ═════════════════
 *
 * Ici `MONGO_MAX_POOL_SIZE` vaut **100 par défaut** (`src/db.js`), et le
 * commentaire qui le pose le dit lui-même : « Le dimensionner suppose des
 * mesures que nous n'avons pas encore ». Ce sont ces mesures. 100 est-il trop
 * haut — auquel cas chaque instance réserve cent connexions contre Atlas, et la
 * limite du plan tombe dès qu'on multiplie les instances — ou trop bas ? Sans
 * mesure, monter ou descendre ce chiffre est une superstition.
 *
 * ⚠️ Ne pas transposer le 15 mesuré sur Tx-Core (campagne du 2026-08-28,
 * `BENCHMARKS.md` §8.4) : ce sont deux services, deux profils de requêtes et
 * deux clusters. Le chiffre de Tx-Core dit comment LIRE ces jauges, il ne dit
 * pas quelle valeur poser ici.
 *
 ...
 *
 * Elle nomme le POOL, pas la base. Cette passerelle ouvre deux connexions
 * Mongoose (`gateway` via `mongoose.connect`, `users` via
 * `mongoose.createConnection`) qui ne partagent PAS leur `MongoClient` : deux
 * pools distincts, donc deux séries, `pool="gateway"` et `pool="users"`,
 * chacune avec son propre plafond de 100. C'est précisément pour cela qu'il faut
 * les distinguer — la saturation peut n'affecter qu'un des deux, et une série
 * agrégée la noierait.
 *
 * `trackPool` dédoublonne malgré tout par identité de client : si un jour les
 * deux connexions venaient à partager un client, il n'y aurait qu'une série
 * (`gateway+users`) au lieu d'un double comptage silencieux.
 *
 * ═══ CARDINALITÉ ═════════════════════════════════════════════════════════
 *
